...
<tr>
	...
	<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="product-title">{$product.product|unescape}</a>{if !$product.exclude_from_calculate}<a class="{$ajax_class} icon-delete-big" href="{"checkout.delete?cart_id=`$key`&amp;redirect_mode=`$mode`"|fn_url}" rev="cart_items,checkout_totals,cart_status*,checkout_steps,checkout_cart" title="{__('remove')}"></a>{/if}	
	...
	<td class="quantity-cell center{if $product.is_edp == "Y" || $product.exclude_from_calculate} quantity-disabled{/if}">
		...
		<div class="quantity cm-reload-{$obj_id}{if $settings.Appearance.quantity_changer == "Y"} changer{/if}" id="quantity_update_{$obj_id}">
			...
			{else}
				...
				{/if}
				<input type="text" size="3" id="amount_{$key}" name="cart_products[{$key}][amount]" value="{$product.amount}" class="input-text-short cm-amount"{if $product.qty_step > 1} data-ca-step="{$product.qty_step}"{/if} />abcde
				{if $settings.Appearance.quantity_changer == "Y"}
...
{/capture}
{include file="common_templates/mainbox_cart.tpl" title=__('cart_items') content=$smarty.capture.cartbox}
