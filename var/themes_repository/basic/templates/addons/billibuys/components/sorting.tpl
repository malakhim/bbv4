...
<span class="cm-dropdown-title sort-dropdown dropdown-wrap-left"><a class="dropdown-wrap-right">{$pagination.items_per_page} {__('per_page')}</a></span>
	<ul class="cm-dropdown-content">
		{foreach from=$pagination.product_steps item="step"}
		{if $step != $pagination.items_per_page}
			<li><a class="{$ajax_class}" href="{"`$range_url`&amp;items_per_page=`$step`"|fn_url}" rev="{$pagination_id}">{$step} {__('per_page')}</a></li>
		{/if}
		{/foreach}
	</ul>
</div>
