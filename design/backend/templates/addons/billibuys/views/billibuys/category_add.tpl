...
<div id="content_detailed">
	...
	{include file="common_templates/subheader.tpl" title=__('general')}

	<div class="form-field">
		<label for="category" class="cm-required">{__('name')}:</label>
	...
	<div class="form-field">
		{if "categories"|fn_show_picker:$smarty.const.CATEGORY_THRESHOLD}
			<label class="cm-required" for="location_category_id">{__('location')}:</label>
			{include file="pickers/categories_picker.tpl" data_id="location_category" input_name="category_data[parent_category_id]" item_ids=$category_data.parent_category_id|default:"0" hide_link=true hide_delete_button=true show_root=true default_name=__('root_level') display_input_id="location_category_id" except_id=$id}
		{else}
			<label for="category_data_parent_id">{__('location')}:</label>
			<select	name="category_data[parent_category_id]" id="category_data_parent_id">
				<option	value="0" {if $category_data.parent_category_id == "0"}selected="selected"{/if}>- {__('root_level')} -</option>
	...
	<div class="form-field">
		<label for="cat_descr">{__('description')}:</label>
	...
	<div class="form-field">
		...
		<div class="float-left">
			{include file="common_templates/attach_images.tpl" image_name="category_main" image_object_type="category" image_pair=$category_data.main_pair image_object_id=$id icon_text=__('text_category_icon') detailed_text=__('text_category_detailed_image') no_thumbnail=true}
		</div>
	</div>

	</fieldset>
{*
	{include file="common_templates/subheader.tpl" title=__('seo_meta_data')}
	...
	*}
	{*
	{include file="common_templates/subheader.tpl" title=__('availability')}
	<div class="form-field">
		<label>{__('usergroups')}:</label>
			<div class="select-field">
				...
				<p><label for="usergroup_to_subcats">{__('to_all_subcats')}</label>
	...
	<div class="form-field">
		<label for="category_position">{__('position')}:</label>
...
<div id="content_layout">
	...
	<div class="form-field">
		<label for="category_default_layout">{__('product_details_layout')}:</label>
	...
	<div class="form-field">
		<label for="use_custom_templates">{__('use_custom_layout')}:</label>
	...
	<div class="form-field">
		<label for="category_product_columns">{__('product_columns')}:</label>
	...
	<div class="form-field">
		<label for="available_layouts">{__('available_layouts')}:</label>
	...
	<div class="form-field">
		<label for="category_default_layout">{__('default_category_layout')}:</label>
...
{/capture}
{include file="common_templates/mainbox.tpl" title=__('add_categories') content=$smarty.capture.mainbox}