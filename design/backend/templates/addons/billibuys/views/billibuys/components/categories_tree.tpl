...
<tr>
	...
	<th class="right" width="15%">{__('requests')}</th>
	...
	{strip}
		<span class="strong" style="padding-left: {$shift}px;">
			{if $category.has_children || $category.subcategories}
				{if $show_all}
					<img src="{$images_dir}/plus.gif" width="14" height="9" border="0" alt="{__('expand_sublist_of_items')}" title="{__('expand_sublist_of_items')}" id="on_{$comb_id}" class="hand cm-combination {if $expand_all}hidden{/if}" />
				{else}
					<img src="{$images_dir}/plus.gif" width="14" height="9" border="0" alt="{__('expand_sublist_of_items')}" title="{__('expand_sublist_of_items')}" id="on_{$comb_id}" class="hand cm-combination" onclick="if (!$('#{$comb_id}').children().get(0)) $.ajaxRequest('{"categories.manage?category_id=`$category.bb_request_category_id`"|fn_url:'A':'rel':'&'}', {$ldelim}result_ids: '{$comb_id}'{$rdelim})" />
				{/if}
				<img src="{$images_dir}/minus.gif" width="14" height="9" border="0" alt="{__('collapse_sublist_of_items')}" title="{__('collapse_sublist_of_items')}" id="off_{$comb_id}" class="hand cm-combination{if !$expand_all || !$show_all} hidden{/if}" />&nbsp;
			{/if}
...
<tr {if $category.level > 0}class="multiple-table-row"{/if}>
	...
	{else}
		...
		<td width="10%" class="nowrap">
			...
			<li><a class="cm-confirm" href="{"categories.delete?category_id=`$category.bb_request_category_id`"|fn_url}">{__('delete')}</a></li>
			{/capture}
